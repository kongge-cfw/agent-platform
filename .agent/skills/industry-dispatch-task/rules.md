...
- 规程以 SKILL.md「规程裁定」为准：出卡前读全源明细，只维护 `pending_write/create.json`。问题处置冻结 `items[].problems` 对象数组，不要手写 Markdown 表，禁止 `dispatch_items.json`。出卡前必须脚本 `check` 退出码 0；确认后脚本 `render` 成四列表再提交。企业合计页不是企业问题。禁止把企业明细或企业 ID 打进对话。
- 企业收到的整改说明是 **Markdown 问题清单**（`## 企业问题` / `## 车辆问题` / `## 驾驶员问题` + 四列表），由 `scripts/validate_create_json.py render` 写入 `items[].requirement`。车辆列：发生时间、车牌号、问题类型、事实。人员列：发生时间、姓名、问题类型、事实。禁止一句话摘要，禁止建议句，禁止把 JSON 数组原样传给 MCP。**企业问题 = 源材料没有精确到具体车辆、也没有精确到具体驾驶员的事项**。有分车/分人不得再写企业合计。车辆对象必须有真实车牌。事实须含次数/里程/完整率等源材料已有数值。禁止编造源附件中没有的车牌、人员、指标。源材料提到或要求整改的，不论大小一律下发；仅源材料明确标注无需处理的才可省略。

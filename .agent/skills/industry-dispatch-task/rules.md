...
- 规程以 SKILL.md「规程裁定」为准：出卡前读全源明细，只维护 `pending_write/create.json`。问题处置冻结 `items[].problems` 对象数组，不要手写 Markdown 表，禁止 `dispatch_items.json`。出卡前必须脚本 `check` 退出码 0；确认后脚本 `render` 成四列表再提交。带「汇总、合计、小计、总计」的加总行不是企业问题。文件里没有车牌、没有姓名的事项要写入企业问题。禁止把企业明细或企业 ID 打进对话。
- 企业收到的整改说明是 **Markdown 问题清单**（`## 企业问题` / `## 车辆问题` / `## 驾驶员问题` + 四列表），由 `scripts/validate_create_json.py render` 写入 `items[].requirement`。车辆列：发生时间、车牌号、问题类型、事实。人员列：发生时间、姓名、问题类型、事实。禁止一句话摘要，禁止建议句，禁止把 JSON 数组原样传给 MCP。**企业问题 = 源材料没有精确到具体车辆、也没有精确到具体驾驶员的事项**。有分车/分人时仍要写入文件中的企业级事项，只不写加总行。车辆对象必须有真实车牌。事实须含次数/里程/完整率等源材料已有数值。禁止编造源附件中没有的车牌、人员、指标。源材料提到或要求整改的，不论大小一律下发；仅源材料明确标注无需处理的才可省略。没有问题明细的企业不要进入 `enterpriseNames`、`items` 和确认卡；`check` 会删掉这类企业。确认卡企业数和清单只能抄 `check` 输出。
- 出卡前必须 `Read pending_write/create.json` 并用 Bash 跑 `validate.py check`：必须是对象数组，**一车一条**或**一驾驶员一条**。禁止 Read `.py`。禁止 Edit `pending_write/`，改文件一律 Write 整份 JSON。禁止概要、统计和多对象挤一条。不通过则 Write 覆盖同一 JSON，禁止出卡。
- 按证据来源定性：执法认定=违法事实，设备报警=监测预警，检查发现=隐患，证照有效期=资质异常。禁止把报警线索直接写成违法，禁止在无调查结论时编造根因。
- 任务级 `requirement` 用 SKILL.md 按类型固定句，不进确认卡。企业明细冻结 `problems`，不要分析/建议三节，不能用三句摘要代替对象数组。问题整改默认 `needAudit=true`。身份证号、资格证号等敏感标识默认脱敏。
- 下发结果使用固定 Markdown 回执。企业明细为 Markdown，由任务系统展示。
- 确认卡取消后禁止调用任何写入 MCP，禁止立刻再弹确认卡。
- 确认卡字段恰好 6 个，顺序固定为：任务名称、任务类型、完成时限、企业提交后需行业审核、可下发企业数、可下发企业清单。缺 `enterpriseCount` 或 `enterprises` 禁止出卡。两字段只含可下发企业（点名：`found=true && enabled=true && matchCount=1`；圈选：`enabled=true`）。无法匹配、停用、重名不进家数、不进名单，总家数只写入 `risk_note`。`enterpriseCount` 必须为 `N家`，N 只能是 `check` 输出 `可下发企业数=` 后面的整数，禁止按 `、` 重数。`taskType` 必须是中文下拉：`value_type=enum`，`options=["通知", "工作部署", "问题处置", "材料报送"]`。确认后 create 必须带卡上中文原值。`enterprises` 用 `value_type=string`，值原样粘贴 `check` 的 `可下发企业清单=` 等号后整段，禁止换行、编号和重排。禁止增加第七字段。确认后禁止再 Write `create.json`。`render` 用 `--name` `--task-type` `--need-audit` `--deadline` 盖上卡上四个字段（内部已含 check 与 check-submit），再提交 `pending_write/submit.json`，禁止提交 `dispatch_items.json`，禁止改写 facts。
...
- summary 只允许使用 SKILL.md 的立即下发句式；risk_note 只允许使用两个立即下发固定句式；家数写 `N家`；summary 中的时限写 `yyyy-MM-dd` 或 `不限期`。确认卡 `deadlineDate` 的 `value_type` 必须始终为 `date`：有时限填 `yyyy-MM-dd`，无时限 `value` 为空，禁止把「不限期」写入字段值。可下发企业名单只粘贴 `check` 的 `可下发企业清单=`，禁止按用户原始顺序重拼。
...
- 确认卡只展示名称。企业 ID 必须在出卡前由 resolve/list 取得并冻结进 `create.json`。确认后不再反查企业，只读取冻结 JSON。禁止把名称当 ID，JSON 里的 ID 必须加引号。

...
- 规程以 SKILL.md「规程裁定」为准：出卡前读全源明细，只维护 `pending_write/create.json`。问题处置冻结 `items[].problems` 对象数组，不要手写 Markdown 表。确认后只 Read 这一份 JSON，按模板拼成 `requirement` 再提交。企业合计页不是企业问题。禁止把企业明细或企业 ID 打进对话。
- 企业收到的整改说明是 **Markdown 问题清单**（`## 企业问题` / `## 车辆问题` / `## 驾驶员问题` + 表），由冻结的 `problems` 按 SKILL.md 模板拼成后写入 `items[].requirement`。禁止一句话摘要，禁止建议句，禁止把 JSON 数组原样传给 MCP。**企业问题 = 源材料没有精确到具体车辆、也没有精确到具体驾驶员的事项**。有分车/分人不得再写企业合计。车辆对象必须有真实车牌。事实须含次数/里程/完整率等源材料已有数值。禁止编造源附件中没有的车牌、人员、指标。源材料提到或要求整改的，不论大小一律下发；仅源材料明确标注无需处理的才可省略。
- 出卡前必须 `Read pending_write/create.json` 检查每个 `items[].problems`：必须是对象数组，**一车一条**或**一驾驶员一条**。禁止概要、统计和多对象挤一条。不通过则覆盖同一 JSON，禁止出卡。

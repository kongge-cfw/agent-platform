...
- 封闭选项用 **enum**，并提供 `options`（中文标签字符串，或 `{value,label}`）。提交值必须是选项里的 value，不要把枚举写成可自由输入的 string。
- 收到「【业务确认】用户已确定」：继续**原任务**，不是新问题。若本轮已启用技能或存在 [HITL 续跑上下文]，必须按该 SKILL.md workflow 执行；按快照写入前核对已解析 ID，快照只有名称时先解析再写。禁止臆造外部主键。确认前已生成的待写入快照必须原样提交；没有快照时先读取附件或工具结果，禁止凭记忆编造标识、人员、数值或明细行。
- 确认后需要原样提交的长文、明细或结构化载荷，出卡前写入当前工作区 **pending_write/** 目录（不要作为对话正文、不要登记为用户产物）。确认回执后优先读取这些文件再写入。
...
- 收到「【用户回答】」回执后，按回执中的选项和补充说明继续**原问题/原技能流程**；不要把回执当作新的独立问题，也不要再次询问已经回答的字段。若存在 [HITL 续跑上下文] 或已启用技能，必须按该流程继续。
...
- 调用 **ask_user_question** 时入参必须是 JSON 对象：`question`（一个问题）+ `options`（2-12 项，每项含 id/label）。禁止把选项数组、`questions` 列表或 XML 参数片段当作整个入参；背景说明写入 `context` 且尽量简短。"""
        ...
        *,
        ...
        if office_read_tools:
            table_rows.append(
                ...
                "excel_document_read 的 action 只能是 inspect、profile、filter 或 read_range，不要传 read；"
                "分析、统计或大表必须先 profile（可省略 sheet_name 以扫描全部工作表）；"
                "分析结论只依据 profile/filter 返回的 top_values 与各值 count；"
                "低基数分类列最多 200 项（可能已是全量，看 top_values_complete）；"
                "高基数列仍最多 50 项，必须同时看 unique_count、omitted_unique、top_values_row_coverage；"
                "top_values 未列出的对象禁止当成 0 条，必须再用 filter 且专有全称用 op=eq；"
                "matched_count 是筛选条件命中的总行数，禁止把它记成某一个名称的条数；"
